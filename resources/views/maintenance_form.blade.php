            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 leading-tight">
                        {{ isset($maintenance_record) ? 'Edit Maintenance Record' : 'Add Maintenance Record' }}
                    </h1>
                    <p class="text-gray-500 text-sm mt-0.5">
                        {{ isset($maintenance_record) ? 'Update the details below' : 'Log service events, repairs, and costs' }}
                    </p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <form action="{{ isset($maintenance_record) ? route('maintenance_records.update', $maintenance_record->id) : route('maintenance_records.store') }}"
                          method="POST" class="px-6 py-5 space-y-5">
                        @csrf
                        @if(isset($maintenance_record)) @method('PUT') @endif
                                ...
                                <select id="vehicle_id" name="vehicle_id"
                                    ...
                                    @foreach($vehicles as $vehicle)
                                        <option value="{{ $vehicle->id }}"
                                            {{ (isset($maintenance_record) && $maintenance_record->vehicle_id == $vehicle->id) || old('vehicle_id') == $vehicle->id ? 'selected' : '' }}>
                        ...
                        <div class="fm-field">
                            ...
                            <input type="text" id="issue" name="issue"
                                   value="{{ old('issue', $maintenance_record->issue ?? '') }}"
                                   class="{{ $errors->has('issue') ? 'is-invalid' : '' }}"
                                   placeholder="e.g. Oil Change, Brake Replacement" required>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                            ...
                            <div class="fm-field">
                                ...
                                <input type="number" id="cost" name="cost"
                                       value="{{ old('cost', $maintenance_record->cost ?? '') }}"
                            ...
                            <div class="fm-field">
                                ...
                                <input type="text" id="technician_name" name="technician_name"
                                       value="{{ old('technician_name', $maintenance_record->technician_name ?? '') }}"
                    ...
                    <div class="divide-y divide-gray-50 px-5">
                        <div class="py-3 flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="font-mono text-sm font-bold text-blue-700">{{ $maintenance_record->vehicle->plate_number ?? 'N/A' }}</p>
                            </div>
                        </div>
                        <div class="py-3 flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-xs text-gray-400 font-medium">Issue</p>
                                <p class="text-sm font-semibold text-gray-900">{{ $maintenance_record->issue }}</p>
                            </div>
                        </div>
                        <div class="py-3 flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-xs text-gray-400 font-medium">Service Date</p>
                        ...
                        <div class="py-3 flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-xs text-gray-400 font-medium">Cost</p>
                                <p class="text-sm font-bold text-gray-900">${{ number_format($maintenance_record->cost, 2) }}</p>
                            </div>
                        </div>
                        <div class="py-3 flex items-center gap-3">
                            <div class="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                            ...
                            <div>
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-50 text-amber-700 text-xs font-semibold rounded-full border border-amber-200">
